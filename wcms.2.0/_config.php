...
/**
 * 접속국가 허용설정 : 이성준 - 2013년 7월 10일 수요일 (GEO_IP 확장모듈 설치)
 */
if($cfg['site']['allowCountry'] && $cfg['site']['allowCountry'] != "ALL" && !preg_match('/_Admin/i', $_SERVER['PHP_SELF']))
{
    /* GEO_IP 확장모듈에서 국가코드를 얻어온다 : 사설IP는 제외해야 오류가 발생하지 않는다 */
    $cfg['geoip'] = (preg_match('/^10|^172|^192/', $_SERVER['REMOTE_ADDR'])) ? "KR" : geoip_country_code_by_name($_SERVER['REMOTE_ADDR']);
    if($cfg['site']['allowCountry'] == '1' && $cfg['geoip'] != 'KR')
    {
        /* 국내만 허용 */
        header("HTTP/1.0 404 Not found");
        exit; 
    } 
    else if($cfg['site']['allowCountry'] == '2' && ($cfg['geoip'] != 'KR' && $cfg['geoip'] != 'US'))
    {
        /* 국내와 미국만 허용 */
        header("HTTP/1.0 404 Not found"); 
        exit; 
    } 
    else if($cfg['site']['allowCountry'] == '3' && ($cfg['geoip'] == 'CN' || $cfg['geoip'] == 'RU'))
    {
        /* 스팸발송지가 심한 지역(중국,러시아)만 제한 */
        header("HTTP/1.0 404 Not found"); 
        exit; 
    }
}
