...
//	넘어온 값과 변수 동기화 및 validCheck
	...
	$db->data['name'] = $name;
	$db->data['form'] = $form;
	$db->data['listing'] = htmlspecialchars($name);
	$db->data['useHidden'] = $useHidden;
	$db->data['config']['module'] = "sns";
	...
	$db->data['config']['facebookId'] = $facebookId;
	$db->data['config']['facebookHeader'] = $facebookHeader;
	$db->data['config']['facebookBorder'] = $facebookBorder;
	$db->data['config']['twitterId'] = $twitterId;
	$db->data['config']['twitterWidgetId'] = $twitterWidgetId;
	...
	if($db->sqlInsert("display__".$skin, "REPLACE", 0) > 0)
	{
		...
		{
			$func->errCfm("SNS API 유닛정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#skinSelector', '".$cfg['droot']."_Admin/modules/displayList.php?type=displayList&mode=design&skin=".$_POST['skin']."&position=".$_POST['position']."');");
		} else
		{
			$func->err("SNS API 유닛정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#tabBody".$_POST['position']."', './modules/displayList.php?type=displayList&skin=".$_POST['skin']."&position=".$_POST['position']."',null,300);");
		}
	}
}
#--- 디스플레이 수정
...
else if($_GET['type'] == "displayDel" && $_GET['sort'] && $_GET['position'])
{
	...
	if($db->getAffectedRows() > 0)
	{
		if(!preg_match('/\/_Admin\//', $_SERVER['HTTP_REFERER']))
		{
			$func->errCfm("SNS API 유닛이 정상적으로 삭제되었습니다.", "parent");
		}
		else
		{
			$func->ajaxMsg("SNS API 유닛이 정상적으로 삭제되었습니다.","$.insert('#tabBody".$_GET['position']."', './modules/displayList.php?type=displayList&skin=".$_GET['skin']."&position=".$_GET['position']."',null,300)", 20);
		}
	}
}
else
...
?>
<div id="messageBox"><strong>Message :</strong>&nbsp;&nbsp;&nbsp;</div>
...
<div class="menu_violet">
	<p title="드래그하여 이동하실 수 있습니다"><strong>SNS API 유닛설정</strong> ( 레이아웃:<?php echo($_GET['skin']);?> / 위치:<?php echo($_GET['position']);?> )</p>
</div>
<table class="table_list" style="width:100%;">
	...
	$form->addStart('SNS 노출타입', 'name', 1, 0, 'M');
	$form->add('radio', array('facebookLikebox'=>'페이스북','twitterTimeline'=>'트위터'), $Rows['name'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>SNS 노출타입 설정</span>');
	$form->addEnd(1);

	$form->addStart('페이스북 ID', 'facebookId', 1);
	$form->add('input', 'facebookId', $config['facebookId'], 'width:310px;','maxlength="16"');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li class="opt"><span>Facebook 아이디 입력</span>');
	$form->addEnd(1);

	$form->addStart('페이스북 헤더', 'facebookHeader', 1);
	$form->add('radio', array('true'=>'노출함','false'=>'노출안함'), $config['facebookHeader'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>페이스북 헤더 노출 여부 설정</span>');
	$form->addEnd(1);

	$form->addStart('페이스북 라인', 'facebookBorder', 1);
	$form->add('radio', array('true'=>'노출함','false'=>'노출안함'), $config['facebookBorder'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>페이스북 Border라인 노출 여부 설정</span>');
	$form->addEnd(1);

	$form->addStart('트위터 ID', 'twitterId', 1);
	$form->add('input', 'twitterId', $config['twitterId'], 'width:310px;','maxlength="16"');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li class="opt"><span>Twitter 아이디 입력</span>');
	$form->addEnd(1);

	$form->addStart('트위터 위젯 ID', 'twitterId', 1);
	$form->add('input', 'twitterWidgetId', $config['twitterWidgetId'], 'width:310px;','maxlength="30"');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li class="opt"><span>Twitter 위젯 아이디 입력(트위터 설정에서 위젯생성)</span>');
	$form->addEnd(1);
	...
	$form->add('input', 'bgColor', $config['bgColor'], 'width:60px;', 'maxlength="7"', 'onclick="colorPickerShow(\'bgColor\', 0);" color="true"');
	...
	$form->add('radio', array('N'=>'노출함','Y'=>'노출안함','H'=>'숨김모드','T'=>'임시모드'), $Rows['useHidden'], 'color:#000;');
	$form->addEnd(0);
