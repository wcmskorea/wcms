...
<table class="table_list">
	...
	<tr>
		...
		<ol>
			...
			<?php
			$db->query(" SELECT cate,name FROM `site__` WHERE skin='".$_GET['skin']."' AND SUBSTRING(cate,1,3)<>'000' ORDER BY cate ASC ");
	...
	$form->add('select', array('Text'=>'상단(좌측) 텍스트(가로 서브메뉴)','TextVertical'=>'상단 텍스트(세로 서브메뉴)','ImgTxt'=>'상단 이미지+텍스트(슬라이드)','TextSlide'=>'상단 텍스트(슬라이드)','Mimage'=>'상단 이미지(모바일)','Fxml'=>'상단 플래시(XML연동)','Image2'=>'좌측 이미지(공통)','Image'=>'좌측 이미지','Category'=>'좌측 카테고리','Widget01'=>'텍스트위젯01(나인갤러리)'), $Rows['listing'], 'width:313px;');
	$form->addEnd(0);
	...
	$form->addStart('라인 색상', 'colorLine', 1);
	$form->class = "input_color";
	$form->add('input', 'colorLine', $config['colorLine'], 'width:60px;', '', 'onclick="colorPickerShow(\'colorLine\', 25);"');
	$form->addHtml('<li class="opt"><span>" # " 포함하여 입력</span></li>');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li class="opt"><span>BOX의 라인색</span>');
	$form->addEnd(1);
	...
	$form->addStart('자동 펼침', 'rollOver', 1, 0, 'M');
	$form->add('radio', array('N'=>'마우스 오버시','C'=>'마우스 클릭시','Y'=>'모두펼침'), $config['rollOver'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>1차 메뉴 오버시 서브메뉴 오픈 설정</span>');
	$form->addEnd(1);
	?>
	</tbody>
</table>
