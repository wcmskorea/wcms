<?php
/* --------------------------------------------------------------------------------------
| Local Navigation : TEXT+SLIDE 방식
|----------------------------------------------------------------------------------------
| Lastest (2013년 7월 22일 화요일 : 이성준)
*/
$this->cate = ($this->caching) ? $this->cacheCate : __CATE__;

$this->result .= '<h2>로컬메뉴</h2>'.PHP_EOL;
$this->result .= '<div class="localNavi02">'.PHP_EOL;
$this->result .= '<div class="menuBox" style="height:'.$this->config['height'].'px">'.PHP_EOL;
$this->result .= '<h2 class="subBox">메인메뉴입니다.</h2>'.PHP_EOL;
...
else
{
	...
	while($sRows = @mysql_fetch_array($rst))
	{
		...
		$this->result .= '<li style="width:'.$width.$unit.'" class="dep1" id="lnb0"><h2><a href="'.$link.'" target="'.$target.'">'.$sRows['name'].'</a></h2>'.PHP_EOL;
		...
		if($GLOBALS['cfg']['skin'] != 'mobile' && $sRows['child'] > 0)
		{
			...
			while($ssRows = @mysql_fetch_array($rst2))
			{
				...
				$this->result .= '<li><a href="'.$link.'" target="'.$target.'" title="'.$ssRows['name'].'">'.$ssRows['name'].'</a></li>'.PHP_EOL;
				$s++;
			}
			unset($s);
