...
else
{
	...
	while($sRows = @mysql_fetch_array($rst))
	{
	    ...
	    {
			$link = $GLOBALS['cfg']['droot'].'index.php?cate='.$sRows['cate'];
			$target = $sRows['target'];
	    }
	    /* 출력 */
	    $sRows['name'] = str_replace("&", "&amp;", $sRows['name']);
	    $this->result .= '<li style="width:'.$width.$unit.';" class="navi" id="lnb'.$sRows['cate'].'"><p><a href="'.$link.'" target="'.$target.'">';
	    ...
	    $this->result .= '<ul id="lnb'.$sRows['cate'].'sub" class="naviSub hide" style="margin-left:'.intval($toWidth-$width).$unit.'; background-color:'.$xml['1'].';">'.PHP_EOL;
        ...
        while($ssRows = @mysql_fetch_array($rst2))
    	{
			...
			$this->result .= '<li class="naviSubMenu">ㆍ<a href="'.$link.'" target="'.$target.'" title="'.$ssRows['name'].'">';
			$this->result .= ($ssRows['cate'] == substr($this->cate, 0, 6)) ? '<span class="orange">'.$ssRows['name'].'</span>' : $ssRows['name'];
...
//<![CDATA[
$(document).ready(function(){$("div.localNavi > ul > .navi > .naviSub").mouseleave(function(){$(this).parent().removeClass("on");$(this).fadeTo("slow", 1).hide();});$.changeTopMenu();});
//]]>
</script>';
?>
