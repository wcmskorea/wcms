...
$this->result .= '<div id="leftNaviText">'.PHP_EOL;
$this->result .= '<ul class="parents">'.PHP_EOL;

$len  = $this->cate ? strlen($this->cate)+3 : 3;
$rst = @mysql_query(" SELECT * FROM `site__` WHERE skin='".$GLOBALS['cfg']['skin']."' AND LENGTH(cate)='".$len."' AND cate LIKE '".$this->cate."%' AND status='normal' ");
$n = 1;
