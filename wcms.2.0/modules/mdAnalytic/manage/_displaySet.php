...
//		$func->validCheck(체크할 값, 항목제목, 항목명, 체크타입)
	...
	$db->data['config']['module'] = "mdAnalytic";
	...
	if($db->sqlInsert("display__".$skin, "REPLACE", 0) > 0)
	{
		...
		{
			$func->errCfm("Display(방문자통계) 위젯 정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#skinSelector', '".$cfg['droot']."_Admin/modules/displayList.php?type=displayList&mode=design&skin=".$_POST['skin']."&position=".$_POST['position']."');");
		}
		else
		{
			$func->err("Display(방문자통계) 위젯 정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#tabBody".$_POST['position']."', './modules/displayList.php?type=displayList&skin=".$_POST['skin']."&position=".$position."',null,300);");
		}
	}
}
//디스플레이 수정
...
<table class="table_list" summary="Display(게시판관리 모듈) 유닛설정" style="width:100%">
	...
	$form->add('select', array('box'=>'기본형'), $Rows['form'], 'width:170px;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li>노출형태 설정');
	$form->addEnd(1);

	$form->addStart('리스트 타입', 'listing', 1, 0, 'M');
	$form->add('select', array('Basic'=>'기본형'), $Rows['listing'], 'width:170px;');
	$form->addEnd(0);
