...
if($sess->decode($_POST[type]) == "findUserAuth") {
	$user = $db->QueryFetch("SELECT * FROM mdMember__account INNER JOIN mdMember__info
													...
													AND mdMember__info.bizNo='".$_POST[bizno]."' AND mdMember__account.email='".$_POST[email]."'");
	if($db->getNumRows() < 1)	{
	...
	} else
	{
		...
		| SMS(Socket) 문자 발송
		*/
		if(in_array('mdSms', $cfg['modules']) && $cfg['module']['find'] == 'sms')
		{
			$sock->tempMode		= "mdMemberReg";
		...
		*/
		...
		$content .= '회 사 명&nbsp;:&nbsp;<strong>'.$user[name].'</strong><br /><br />';
		...
		if($result)
		{
			$db->Query("UPDATE `mdMember__account` SET idcode='".$authcode."' WHERE email='".$_POST[email]."'");
