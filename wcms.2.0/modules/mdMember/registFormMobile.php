...
<input type="hidden" name="checkuseremail" id="checkuseremail" title="이메일 중복체크" value="" />
	...
	$form->class = $form->class.' korean';
	$form->add('input', 'name', $Rows['name'], 'width:120px; ime-mode:active','korean="true" minlength="2" maxlength="10"');
	$form->addEnd(1);

	$form->addStart('아이디', 'id', 1, 0 ,'M');
	$form->add('input', 'id', $Rows['id'], 'width:120px; ime-mode:disabled', 'onblur="$.checkOverLap(\''.$sess->encode("checkUserId").'\',\'Id\');" userid="true"');
	$form->addHtml('<li class="opt pd3"><span id="checkId" class="small_orange">4~15자의 영문을 포함한 숫자, _ 를 혼용가능</span></li>');
	$form->addEnd(1);
	...
	if($cfg['module']['opt_nick'] == 'M') {
		$form->addStart('닉네임', 'nick', 1, 0 , $cfg['module']['opt_nick']);
		$form->add('input', 'nick', $Rows['nick'], 'width:120px;', 'onblur="$.checkOverLap(\''.$sess->encode("checkUserNick").'\',\'Nick\');" nick="true"');
		$form->addHtml('<li class="opt"><span id="checkNick" class="small_orange"></span></li>');
		$form->addEnd(1);
	}

	if($cfg['module']['opt_email'] == 'M') {
		$form->addStart('이메일', 'email', 1, 0 , $cfg['module']['opt_email']);
		$form->add('input', 'email', $Rows['email'], 'width:98%; ime-mode:disabled', 'onblur="$.checkOverLap(\''.$sess->encode("checkUserEmail").'\',\'Email\');" email="true"');
		$form->addHtml('<li class="opt"><span id="checkEmail" class="small_orange"></span></li>');
		$form->addEnd(1);
	}

	if($cfg['module']['opt_address'] == 'M') {
	...
	if($cfg['module']['opt_mobile'] == 'M') {
		$form->addStart('휴대전화', 'mobile', 1, 0 , $cfg['module']['opt_mobile']);
		$form->add('input', 'mobile', $Rows['mobile'], 'width:120px;', 'onblur="$.checkOverLap(\''.$sess->encode("checkUserMobile").'\',\'Mobile\');" mobile="true"');
	...
	if($cfg['module']['opt_mobileAuth'] == 'M') {
