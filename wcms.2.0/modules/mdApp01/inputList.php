...
if($member->checkPerm('1') === false) { 
	$func->err("접근 권한이 없습니다. 로그인 후 이용해주시기 바랍니다.", "back"); 
}
...
//현재 상태별 건수정보
$count = $db->queryFetchOne(" SELECT COUNT(*) FROM `mdApp01__content` WHERE cate='".__CATE__."' AND YEAR(FROM_UNIXTIME(dateReg))='".date('Y')."' AND MONTH(FROM_UNIXTIME(dateReg))='".date('m')."' AND DAY(FROM_UNIXTIME(dateReg))='".date('d')."' ");

//등록인원 체크
$enable = (!$cfg['module']['limitEntry'] || $cfg['module']['limitEntry'] > $count) ? true : false;
...
if($cfg['module']['limitTimeStart'] && $cfg['module']['limitTimeEnd'])
{
	...
	if(date('H') < $cfg['module']['limitTimeStart']) { $enable = false; }
	if(date('H') >= $cfg['module']['limitTimeEnd']) { $enable = false; }
}
?>
<div class="boardInput">
<?php
if($cfg['module']['opt_display'] == 'Y')
{
?>
	<table class="table_basic" summary="문의.상담을 위한 개인정보 입력 항목" style="width:100%;">
	<caption>문의.상담 항목</caption>
	<thead>
	<tr><th class="first"><p class="center">선착순</p></th>
			<th class="first"><p class="center">접수인원</p></th>
			<th class="first"><p class="center">가능인원</p></th>
	</tr>
	<tbody>
	<tr><td><p class="center"><strong class="blue"><?php echo(number_format($cfg['module']['limitEntry']));?> 명</strong></p></td>
			<td><p class="center"><strong class="green"><?php echo(number_format($count));?> 건</strong></p></td>
			<td><p class="center"><strong class="red"><?php echo(number_format($cfg['module']['limitEntry'] - $count));?> 건</strong></p></td>
	</tr>
	</tbody>
	</table>
	<br />
...
if($member->checkPerm('3') === false) 
{
	//$func->err("작성 권한이 없습니다. 회원가입 후 신청하실 수 있습니다.", "back"); 
	echo('<p class="center strong"><a href="/index.php?cate=000002002#module"><img src="/user/default/image/mdApp01/regist.jpg" /></a></p>');
...
?>
	...
	<input type="hidden" name="fileCount" value="<?php echo($Rows['file']);?>" />
		...
		<?php
		if($cfg['module']['announce'])	//2012-12-06 추가
		{
			echo('<div class="frame textContent pd10 bg_gray" style="font-size:11px;">'.stripslashes(nl2br($cfg['module']['announce'])).'</div>');
		}
		if($cfg['module']['opt_agreement'] != 'N')
		{
			...
			<div class="pd5"><span class="colorRed"><input type="checkbox" id="agree2" name="agree2" class="input_check required" title="개인정보 수집 및 취급방침에 동의는 필수선택 입니다." value="Y" />&nbsp;<label for="agree2">개인정보 수집 및 취급방침에 동의 합니다. (필수)</label></span></div>');
		...
		if($cfg['module']['opt_name'] != 'N') {
			...
			$form->add('input', 'name', $contentAdd['name'], 'width:120px; ime-mode:active','minlength="2" maxlength="10"';
			$form->addEnd(1);
		}
		...
		if($cfg['module']['opt_mobileAuth'] && $cfg['module']['opt_mobileAuth'] != 'N') {
			...
			$form->addHtml('<li class="opt"><span><a href="javascript:;" onclick="$.message(\''.$cfg['droot'].'modules/mdMember/widgets/checkCertSms.php?&amp;'.__PARM__.'&amp;type='.$sess->encode('checkCert').'\');" class="btnPack white small"><span>인증요청</span></a></span><span class="small_gray">(인증코드가 문자로 발송됨)</span></li>');
			$form->addEnd(1);
		}

		if($cfg['module']['opt_tel'] && $cfg['module']['opt_tel'] != 'N') {
			$form->addStart('연락처', 'tel', 1, 0 , $cfg['module']['opt_tel']);
			$form->add('input', 'tel', $contentAdd['phone'], 'width:120px;', 'phone="true" maxlength="14"');
			$form->addHtml('<li class="opt" style="padding-top:3px"><span class="small_gray"> (예 : 휴대전화 or 일반전화)</span></li>');
			$form->addEnd(1);
		}
		...
		if($cfg['module']['opt_mobile'] != 'N') {
			...
			$form->addHtml('<li class="opt" style="padding-top:3px"><span class="small_gray"> (예 : 010-1234-1234)</span></li>');
			$form->addEnd(1);
		}
		...
		if($cfg['module']['opt_phone'] != 'N') {
			...
			$form->addHtml('<li class="opt" style="padding-top:3px"><span class="small_gray"> (예 : 02-1234-1234)</span></li>');
			$form->addEnd(1);
		}	
		...
		if($cfg['module']['opt_email'] != 'N') {
			...
			$form->add('input', 'email', $contentAdd['email'], 'width:330px; ime-mode:disabled', 'email="true" maxlength="50"');
		...
		if($cfg['module']['opt_schedule'] != 'N') {
			...
			$form->addStart('일정예약', 'scheduleyear', 1, 0 , $cfg['module']['opt_schedule']);
		...
		/*for($i=1; $i<=20; $i++)
		{
			if($cfg['module']['opt_add'.$i] != 'N')
			{
				...
				if($opt['addEx'])
				{
					$form->addHtml('<li class="opt" style="padding-top:3px"><span class="small_gray">( '.$opt['addEx'].' )</span></li>');
				}
				$form->addEnd(1);
			}
		}*/
		...
		if(!$Rows || $Rows['state'] < 1)
		{
			if($Rows['state'] == '0')
			{
				echo('<div class="pd10 center"><span class="btnPack black medium strong"><button type="submit" onclick="return submitForm(this.form)">위 내용으로 변경하기</button></span></div>');
			} 
			else {
			...
			if($Rows['contentAnswers']) 
			{
				...
				<div class="cube">
				<div class="line">
					<p class="pd5 textContent"><strong>'.nl2br($Rows['contentAnswers']).'</strong></p>
				</div>
				</div>');
			} else {
...
<?php 
		...
		echo('<p class="center strong"><img src="/user/default/image/mdApp01/ending.jpg" /></p>');

		//내역 리스트 출력
		if($cfg['module']['listing'] == 'List') {include __PATH__."modules/".$cfg['cate']['mode']."/list.php"; }

		//echo('<div class="pd10 center"><a href="#none" onclick="$.dialog(\''.$cfg['droot'].'index.php\', \'&amp;'.__PARM__.'&amp;type=search&amp;mode=dialog\',400,165)" class="btnPack white medium strong"><span>신청내역 확인 및 변경</span></a></div>');
	}
} 
?>
</div>
