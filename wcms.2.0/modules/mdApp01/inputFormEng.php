...
<input type="hidden" name="fileCount" value="<?php echo($Rows['file']);?>" />
	...
	if($cfg['module']['opt_name'] != 'N') {
		$form->addStart('Name', 'name', 1, 0, 'M');
		$form->add('input', 'name', $contentAdd['name'], 'width:120px; ime-mode:inactive','minlength="2" maxlength="10"');
		$form->addEnd(1);
	}

	if($cfg['module']['opt_mobile'] != 'N') {
		$form->addStart('Mobilephone', 'mobile', 1, 0 , $cfg['module']['opt_mobile']);
		$form->add('input', 'mobile', $contentAdd['mobile'], 'width:120px;', 'mobile="true" maxlength="14"');
		$form->addHtml('<li class="opt"><span class="small_gray"> (ex: 010-123-1234)</span></li>');
		$form->addEnd(1);
	}

	if($cfg['module']['opt_phone'] != 'N') {
		$form->addStart('Telephone', 'phone', 1, 0 , $cfg['module']['opt_phone']);
		$form->add('input', 'phone', $contentAdd['phone'], 'width:120px;', 'phone="true" maxlength="14"');
		$form->addHtml('<li class="opt"><span class="small_gray"> (ex: 02-123-1234)</span></li>');
		$form->addEnd(1);
	}

	if($cfg['module']['opt_email'] != 'N') {
		$form->addStart('E-mail', 'email', 1, 0 , $cfg['module']['opt_email']);
	...
	if($cfg['module']['opt_content'] != 'N') {
		$form->addStart('Inquiry', 'contents', 1, 0 , $cfg['module']['opt_content']);
		$form->add('textarea', 'content', stripslashes(nl2br($Rows['content'])), 'width:400px; height:100px;');
		$form->addEnd(1);
	}
	?>
	</table>
...
else
{
	...
	if(!$Rows || $Rows['state'] < 1)
	{
		if($Rows['state'] == '0')
		{
			echo('<div class="pd10 center"><span class="btnPack green medium strong"><button type="submit" onclick="return $.submit(this.form);">submit</button></span></div>');
		} else
		{

			echo('<div class="pd10 center"><span class="btnPack black medium strong"><button type="submit" '.$uploadSubmit.'>submit</button></span></div>');
