...
<col>
	...
	$form->name = array('Form'=>'입력폼','FormMobile'=>'입력폼(모바일)','FormEng'=>'입력폼(영문)','List'=>'입력폼+목록','Search'=>'검색폼(분양당첨)','SearchMobile'=>'검색폼(이름+휴대폰)');
	...
	$form->addStart('신청자수 제한', 'limitTimeStart', 1);
	$form->add('input', 'limitEntry', $config['limitEntry'], 'width:30px; text-align:center;','digits="true" maxlength="3"');
	$form->addHtml('<li class="opt gray">명까지</li>');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray">신청가능 인원 제한</td>');
	$form->addEnd(1);

	$form->addStart('신청시간 제한', 'limitTimeStart', 1);
	$form->add('input', 'limitTimeStart', $config['limitTimeStart'], 'width:30px; text-align:center;','digits="true" maxlength="2"');
	$form->addHtml('<li class="opt gray">시 ~ </li>');
	$form->id = 'limitTimeEnd';
	$form->add('input', 'limitTimeEnd', $config['limitTimeEnd'], 'width:30px; text-align:center;','digits="true" maxlength="2"');
	$form->addHtml('<li class="opt gray">시 까지</li>');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray">신청시간 제한</td>');
	$form->addEnd(1);

	$form->addStart('중복신청 제한', 'limitDuple', 1);
	$form->add('radio', array('N'=>'제한안함','Y'=>'제한함'), $config['limitDuple'], 'color:black;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray">신청자 중복 제한 (아이피 및 휴대전화)</td>');
	$form->addEnd(1);
