...
//	데이터 정리
	...
	if($Rows['useSecret'] == "Y")
	{
		...
		$url = '<a href="'.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type='.$sess->encode('secret').'&amp;num='.$Rows['seq'].'">';
	}
	...
	$dir = str_replace($cfg['skin']."/", "default/",__SKIN__)."data/".date("Y",$thumb['regDate'])."/".date("m",$thumb['regDate'])."/m-".$thumb['fileName'];
	...
	<div class="center" style="width:'.$cfg['module']['thumbMsize'].'px;">
		...
		if($cfg['module']['regDate'] != 'N' || $cfg['module']['readCount'] != 'N' ) { 
			echo('<div class="date">');
			if($cfg['module']['regDate'] != 'N') { echo('<span>'.$date.'</span>'); }
			if($cfg['module']['regDate'] != 'N' && $cfg['module']['readCount'] != 'N' ) { echo('/'); }
			if($cfg['module']['readCount'] != 'N' ) { echo('hit: <span class="num">'.$Rows['readCount'].'</span>'); }
			echo('</div>'); 
		}
		echo('<!--<div class="author"><span>'.$Rows['writer'].'</span><span class="commentCount">'.$commentCount.'</span></div>-->
		</div>
	</div>
	</div>');
	$n++;
