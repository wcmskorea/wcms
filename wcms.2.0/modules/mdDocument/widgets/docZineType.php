...
while($Rows	= @mysql_fetch_assoc($rst))
{
	...
	$this->result .= '<li style="padding:'.$this->config['docPad'].' '.$this->config['docPad'].' 0 '.$this->config['docPad'].';">'.$url.'<span class="imgList"><img src="'.$dir.'" alt="'.$thumb['fileName'].'" class="thumbNail" style="width:'.$this->config['imgWidth'].'; height:'.$this->config['imgHeight'].';" onerror="this.src=\''.$GLOBALS['cfg']['droot'].'common/image/noimg_m.gif\'" onmouseover="overClass(this);" onmouseout="overClass(this);" /></span><strong>'.$subject.'</strong></a><p class="content" style="width:'.$contentWidth.'px;">'.$url.$Rows['content'].'</a></p></li>'.PHP_EOL;
	$n++;
}
/*
