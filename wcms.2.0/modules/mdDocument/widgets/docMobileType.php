 ...
 *---------------------------------------------------------------------------------------
 */
$squery				 = ($this->config['useUnion'] == "Y") ? "cate like '".$this->config['share']."%'" : "cate='".$this->config['share']."'";
...
$this->result	.= '<div class="recentBody" style="width:'.$this->config['width'].'; height:'.$this->config['height'].'; background:url('.__SKIN__.'image/background/bg_recent_'.strtolower($this->prefix).'.gif) no-repeat left top; background-color:'.$this->config['bgColor'].';">'.PHP_EOL;
...
while($Rows = @mysql_fetch_assoc($rst))
{
	...
	if($this->config['docLink'] == 'L')  //컨텐츠 클릭시 목록으로 이동 추가(20120522)
		$url 			= '<a href="'.$GLOBALS['cfg']['droot'].'mobile/?cate='.$this->cate.'" title="'.$Rows['subject'].'" target="'.$this->target.'">';
	else {
		if($Rows['useSecret'] == "Y"){ $typeString = 'type='.$GLOBALS['sess']->encode('secret');}
		else{ $typeString = "type=view";}
		$url = '<a
	href="'.$GLOBALS['cfg']['droot'].'mobile/?cate='.$this->cate.'&amp;'.$typeString.'&amp;num='.$Rows['seq'].'&amp;productNum='.$Rows['productSeq'].'" title="'.$Rows['subject'].'" target="'.$this->target.'">';
	}

	$this->result .= '<li class="docTitle" style="width:100%; margin:'.$this->config['docPad'].'"><span>[ '.date("y.m.d", $Rows['regDate']).' ]</span><span class="title">&nbsp;'.$url.'<strong>'.$Rows['subject'].'</strong></a></span>'.$icon.'</li>'.PHP_EOL;
	$n++;
}
while($n < $this->config['docCount'])
{
	$this->result .= '<li class="docTitle" style="width:'.$subjectWidth.'px; margin:0 '.$this->config['docPad'].';"><span>[ '.date("y.m.d").' ]</span>&nbsp;<span class="title" style="color:#999;">'.Functions::cutStr('저희 사이트를 방문해주셔서 감사합니다...', $this->config['cutSubject'],"...").'</span></li>'.PHP_EOL;
	$n++;
}
