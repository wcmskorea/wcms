...
<table class="table_list" summary="Display(게시판관리 모듈) 유닛설정">
	...
	$form->add('select', array('List'=>'목록·리스트','Mobile'=>'목록·리스트(모바일)','ListNo'=>'목록·리스트(순위)','Gallery'=>'갤러리·앨범','Gallery2'=>'갤러리·앨범(추가입력)','Grid'=>'갤러리·앨범(그리드)','Layerd'=>'갤러리·앨범(레이어드)','LayerdZine'=>'갤러리+웹진','Cal'=>'일정·캘린더','Zine'=>'웹진','Zine2'=>'웹진(전체)','News'=>'뉴스','Forum'=>'토론·포럼','Page'=>'문서·페이지','Articles'=>'최근 게시물','Comments'=>'최근 댓글','ListComments'=>'최근 댓글+제목','Product'=>'상품후기','App01'=>'상담','Case'=>'실적·사례','Reply'=>'답변·리스트'), $Rows['listing'], 'width:170px;');
	$form->addEnd(0);
	...
	$form->addStart('슬라이드 효과', 'docEasing', 1, 0, 'B');
	$form->add('radio', array(''=>'사용안함','easeOutBounce'=>'easeOutBounce'), $config['docEasing'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>슬라이드 페이징 버튼 노출 설정</span>');
	$form->addEnd(1);

	$form->addStart('슬라이드 버튼', 'docBtn', 1, 0, 'M');
	$form->add('radio', array('false'=>'노출안함','true'=>'노출함'), $config['docBtn'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>슬라이드 이전,다음 버튼 노출 설정</span>');
	$form->addEnd(1);

	$form->addStart('슬라이드 페이징', 'docPager', 1, 0, 'M');
	$form->add('radio', array('false'=>'노출안함','true'=>'노출함'), $config['docPager'], 'color:#000;');
	$form->addEnd(0);
	$form->addHtml('<td class="small_gray bg_gray"><ol><li><span>슬라이드 페이징 버튼 노출 설정</span>');
	$form->addEnd(1);
