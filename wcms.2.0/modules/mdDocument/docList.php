...
$cfg['module']['sortType'] = ($cfg['module']['sortType']) ? $cfg['module']['sortType'] : "idx";
...
{
	case "category":
		$sql .= ($_GET['shc']) ? " AND category = '".$_GET['shc']."' ORDER BY idx DESC" : " ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case "division":
		$sql .= ($_GET['shc']) ? " AND subject like '[".$_GET['shc']."]%' ORDER BY idx DESC" : " ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case  "all":
		$sql .= " AND (subject like '%".$_GET['shc']."%' OR content like '%".$_GET['shc']."%') ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case "subject":
		$sql .= " AND subject like '%".$_GET['shc']."%' ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case "writer":
		$sql .= " AND writer like '%".$_GET['shc']."%' ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case "trash":
		$sql .= " AND idxTrash>'0' ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
	case "cnt":
	...
	case "date":
		...
		break;
	default :
		$sql .= " ORDER BY ".$cfg['module']['sortType']." DESC";
		break;
}

//검색조건에 따른 게시물수 조회
$articleCount = $db->queryFetchOne(" SELECT count(*) as cnt FROM `".$cfg['cate']['mode']."__content".$prefix."` WHERE idxTrash='0' AND ".$sql);
...
echo('<div class="docInfo">');
	echo('<div class="articleNum">');
		...
		{
			$categoryUrl = $_SERVER['PHP_SELF'].'?cate='.$cfg['module']['cate'].'&amp;type=list&amp;mode='.$_GET['mode'].'&amp;sh=category&amp;shc=';
			echo('&nbsp;&nbsp;&nbsp;<span><select name="shc" id="searchCategory" class="bg_gray">
			<option value="">전체 카테고리</option>');
			$cateClass = explode(",", $cfg['module']['category']);
			foreach($cateClass AS $val) { print ('<option value="'.$val.'"'.($_GET['shc'] == $val ? ' selected':'').'>'.$val.' </option>'); }
			echo('</select></span>');
		}
		//말머리 검색 출력
...
while($Rows = $db->fetch())
{
	...
	if($cfg['module']['opt_division'] != 'N')
	{
		...
		$Rows['subject'] = str_replace("[".$out[1][0]."]", '<span style="color:'.$cfg['headColor'][$out[1][0]].'">['.$out[1][0].']</span>', $Rows['subject']);
	}
	if($cfg['module']['opt_category'] != 'N' && $cfg['module']['viewCategory'] == 'Y' && $Rows['category'])
	{
		$Rows['subject'] = '['.$Rows['category'].']'.$Rows['subject'];
	}
...
$(document).ready(function(){
	...
	$("#searchCategory").change(function(){
		location.href = "<?php echo($categoryUrl);?>" + $(this).val();
	});
