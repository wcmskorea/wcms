	...
	function add($element, $name=null, $value=null, $style=null, $valid=null, $addAtt=null)
	{
		...
		switch($this->element)
		{
			...
			case 'input' :
				...
				$this->rst	.= ' id="'.str_replace("[]",null,$this->name).'" name="'.$this->name.'" alt="'.$this->label.'" class="'.$this->class.'" style="'.$this->style.'" '.$this->addAtt.' value="'.$this->value.'"';
			...
			case 'checkbox' :
				...
				$this->rst	.= '<span class="keeping"><input type="checkbox" id="'.$this->id.'" name="'.$this->name.'" alt="'.$this->label.'" class="input_check'.($this->require == 'M' ? ' required' : '').'" '.$this->addAtt.' value="Y"';
			...
			case 'checkboxs' : //$this->name을 배열로 받는다.
				...
				{
					$this->rst	.= ($this->type == 'table') ? '<li class="opt">' : null;
					$this->rst	.= '<span class="keeping"><input type="checkbox" id="'.$this->id.$key.'" name="'.$this->id.'[]" alt="'.$this->label.'" class="input_check'.($this->require == 'M' ? ' required' : '').'" '.$this->addAtt.' value="'.$key.'"';
			...
			case 'radio' : //$this->name을 배열로 받는다.
				...
				foreach($this->name AS $key=>$val)
				{
					...
					$val = trim($val);
					$this->rst	.= ($this->type == 'table') ? '<li class="opt">' : null;
					$this->rst	.= '<span class="keeping"><input type="radio" id="'.$this->id.$prefix.'" name="'.$this->id.'" class="input_check'.($this->require == 'M' ? ' required' : '').'" '.$this->addAtt.' value="'.$key.'"';
			...
			case 'select' :
				$this->rst	.= ($this->type == 'table') ? '<li class="opt">' : null;
				$this->rst	.= '<select id="'.$this->id.'" name="'.$this->id.'" class="bg_gray" '.$this->addAtt.' style="'.$this->style.'" onchange="this.style.color=\'#ff3300\'">';
				/* 배열일때 */
			...
			case 'selectValue' :
				$this->rst	.= ($this->type == 'table') ? '<li class="opt">' : null;
				$this->rst	.= '<select id="'.$this->id.'" name="'.$this->id.'" class="bg_gray" '.$this->addAtt.' style="'.$this->style.'">';
				foreach($this->name AS $key=>$val)
				{
					//$key = (is_numeric($key)) ? $val : $key;
					$val 		= trim($val);
			...
			case 'textarea' :
				$this->rst	.= ($this->type == 'table') ? '<li class="opt">' : null;
				$this->class = ($this->require == 'M') ? $this->class.' required' : $this->class;
				$this->rst	.= '<textarea id="'.$this->id.'" name="'.$this->name.'" alt="'.$this->label.'" style="'.$this->style.'" class="'.$this->class.'" '.$this->addAtt;
				$this->rst	.= ($this->valid) ? ' '.$this->valid.'>' : '>';
			...
			case 'file' :
				...
				$this->rst	.= ' id="'.$this->id.'" name="'.$this->name.'" alt="'.$this->label.'" class="'.$this->class.'" style="'.$this->style.'" '.$this->addAtt.' value="'.$this->value.'"';
				$this->rst	.= ($this->require == 'M') ? ' req="required"' : null;
				$this->rst	.= ($this->valid) ? ' '.$this->valid.' />' : ' />';
				$this->rst	.= ($this->type == 'table') ? '</li>' : null;
				break;
