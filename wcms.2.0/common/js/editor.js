...
var Config = {
	...
	template : {
		all : [ "source", "bar", "save", "preview", "bar", "all", "undo",
				"redo", "bar", "font", "size", "bar", "color", "hilite", "bar",
				"bold", "italic", "underline", "strike", "bar", "sup", "sub",
				"bar", "clean", "del", "bar", "outdent", "indent", "bar",
				"ul1", "ul2", "bar", "left", "center", "right", "justify",
				"bar", "hr", "link", "unlink", "bar", "table", "image", "media" ],
		content : [ "source", "bar", "save", "preview", "bar", "undo", "redo",
				"bar", "font", "size", "bar", "color", "hilite", "bar", "bold",
				"italic", "underline", "strike", "bar", "outdent", "indent",
				"bar", "ul1", "ul2", "bar", "left", "center", "right",
				"justify", "bar", "hr", "link", "unlink", "bar", "table",
				"image", "media" ],
		bbs : [ "preview", "bar", "font", "size", "color", "hilite", "bar",
				"bold", "italic", "underline", "strike", "bar", "left",
				"center", "right", "justify", "bar", "ul1", "ul2", "bar",
				"link", "table", "image", "media", "bar", "source" ],
		html : [ "source", "bar", "font", "size", "bar", "bold", "italic",
				"underline", "strike", "color", "hilite", "bar", "left",
				"center", "right", "justify", "bar", "hr", "link", "bar",
				"table", "image", "media", "bar", "preview" ],
		simple : [ "font", "size", "bar", "bold", "italic", "underline",
				"strike", "color", "hilite" ],
...
editor.prototype.init = function() {
	...
	this._text.style.background = "#fff url(" + this.rootPath + this.imagePath
			+ "/source_bg.gif) 0 -2px";
	...
	this._doc.open();
	this._doc.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="ko" lang="ko"><head><link rel="stylesheet" href="'
					+ this.rootPath
					+ 'common/css/default.css" type="text/css" charset="'
					+ this.charset
					+ '" media="all" /><link rel="stylesheet" href="'
					+ this.cssPath
					+ '" type="text/css" charset="'
					+ this.charset
					+ '" media="all" /><style type="text/css">'
					+ this.css
					+ '</style></head><body class="textContent">'
					+ this.textarea.value + '</body></html>');
	this._doc.close();
	...
	for ( var i = 0; i < arr.length; i++) {
		...
		if (order == "bar") {
			...
			elm.onclick = function() {
				self.commandExec(this, this.commandExec);
			};
...
editor.prototype.commandExec = function(button, order, value) {
	...
	case "hyperlink":
		...
		var html = "<a href='" + self._linkText.value
	...
	case "preview":
		...
		var w = window.open("", "preview", "width=" + 800 + ",height=" + 600
				+ ",top=" + winTop + ",left=" + winLeft
				+ ",status=1,scrollbars=1,resizable=1");
		w.document.open();
		w.document.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="ko" lang="ko"><head><link rel="stylesheet" href="'
						+ this.rootPath
						+ 'common/css/default.css" type="text/css" charset="'
						+ this.charset
						+ '" media="all" /><link rel="stylesheet" href="'
						...
						+ '" media="all" /><style type="text/css">'
						+ this.css
						+ '</style></head><body onclick="self.close()" style="cursor:pointer;" class="textContent">'
						+ self.getHtml() + '</body></html>');
		w.document.close();
		break;
	case "table":
		this.newWindow(this.rootPath + "modules/mdEditor/table.php?cate=" + this._id.replace("editor", ''), "table", "400", "220", "no", "no");
		break;
	case "image":
		this.newWindow(this.rootPath + "modules/mdEditor/upImg.php?cate=" + this._id.replace("editor", ''), "image", "350", "200", "no", "no");
		break;
	case "media":
		this.newWindow(this.rootPath + "modules/mdEditor/upMedia.php?cate=" + this._id.replace("editor", ''), "media", "350", "200", "no", "no");
		break;
	case "plus":
	case "minus":
	...
	case "code":
		self
				.innerHTML('<div style="border:2px #d2d2d2 dotted; background:#eee; padding:.5em; text-align:justify;"></code>');
		break;
	default:
		if (order == "strike")
			order = "strikethrough";
			...
			order = "justify" + order;
		else if (order == "hilitecolor" && util.isIE)
			order = "backcolor";
...
editor.prototype.getHtml = function() {
	...
	html = this.replaceLower(html);
	html = this.selfClosingTag(html);
	this.textarea.value = html;
	return html;
};

editor.prototype.replaceTag = function(content, from, to) {
	return content.replace(new RegExp("(</?)" + from + "(>|\\s+[^>]*>)", "ig"), "$1" + to + "$2");
};

editor.prototype.replaceLower = function(content) {
	return content.replace(/<(\/?)(strong|br|hr|img|p|div|font|table|thead|tbody|tr|td|ul|li|ol|dl|dt|dd|span|style)([^>]*?)>/img, function(e) {
		return e.toLowerCase();
	});
};

editor.prototype.selfClosingTag = function(content) {
...
editor.prototype.hideDiv = function() {
	...
	for ( var i = 0; i < arrTable.length; i++) {
		try {
			eval('util._' + arrTable[i] + 'Table.style.display="none"');
		} catch (e) {
		}
	}
};
...
editor.prototype.fontTable = function() {
	...
	for ( var i = 0; i < font.length; i++) {
		...
		s += "<a href='javascript:;' onclick=\"util._editor.commandExec(null, util.order,'"
				+ font[i]
				+ "');\" style='font:10pt "
				+ font[i]
				+ ";line-height:170%'>" + txt + " (" + font[i] + ")</a><br />";
	}
...
editor.prototype.colorTable = function(order) {
	...
	var col_SelList1 = new Array('#008000', '#993366', '#cc9900', '#9b18c1',
			'#ff9900', '#0000ff', '#ff0000', '#177fcd', '#ff3399', '#8e8e8e');
	var col_SelList2 = new Array('#ffdaed', '#ff0000', '#99dcff', '#0000ff',
			'#a6ff4d', '#009966', '#e4ff75', '#8e8e8e', '#e4e4e4', '#333333');
	var col = new Array();
	col[0] = new Array("#ffffff", "#e5e4e4", "#d9d8d8", "#c0bdbd", "#a7a4a4",
			"#8e8a8b", "#827e7f", "#767173", "#5c585a", "#000000");
	col[1] = new Array("#fefcdf", "#fef4c4", "#feed9b", "#fee573", "#ffed43",
			"#f6cc0b", "#e0b800", "#c9a601", "#ad8e00", "#8c7301");
	col[2] = new Array("#ffded3", "#ffc4b0", "#ff9d7d", "#ff7a4e", "#ff6600",
			"#e95d00", "#d15502", "#ba4b01", "#a44201", "#8d3901");
	col[3] = new Array("#ffd2d0", "#ffbab7", "#fe9a95", "#ff7a73", "#ff483f",
			"#fe2419", "#f10b00", "#d40a00", "#940000", "#6d201b");
	col[4] = new Array("#ffdaed", "#ffb7dc", "#ffa1d1", "#ff84c3", "#ff57ac",
			"#fd1289", "#ec0078", "#d6006d", "#bb005f", "#9b014f");
	col[5] = new Array("#fcd6fe", "#fbbcff", "#f9a1fe", "#f784fe", "#f564fe",
			"#f546ff", "#f328ff", "#d801e5", "#c001cb", "#8f0197");
	col[6] = new Array("#e2f0fe", "#c7e2fe", "#add5fe", "#92c7fe", "#6eb5ff",
			"#48a2ff", "#2690fe", "#0162f4", "#013add", "#0021b0");
	col[7] = new Array("#d3fdff", "#acfafd", "#7cfaff", "#4af7fe", "#1de6fe",
			"#01deff", "#00cdec", "#01b6de", "#00a0c2", "#0084a0");
	col[8] = new Array("#edffcf", "#dffeaa", "#d1fd88", "#befa5a", "#a8f32a",
			"#8fd80a", "#79c101", "#3fa701", "#307f00", "#156200");
	col[9] = new Array("#d4c89f", "#daad88", "#c49578", "#c2877e", "#ac8295",
...
editor.prototype.setLink = function() {
	...
	var id = "_linkText";
	var s = "<input type='text' class='input_text' value='http://' style='width:230px;font:8pt 돋움;color:gray' id='"
			+ id + "' /><p style='margin-top:3px' align=right>";
	s += "링크주소(URL)를 입력하세요 <img src='"
			+ this.rootPath
			+ "image/button/btn_confirm.gif' border=0 onclick=\"util._editor.commandExec(null, util.order, '');\" style='cursor:pointer' align=absmiddle>";
...
editor.prototype.framResize = function(type) {
	...
	case "minus":
		var height = parseInt(this.height.replace(/px$/, "")) - 200;
		if (parseInt(this._iframe.style.height.replace(/px$/, "")) > 200) {
			this._iframe.style.height = height + "px";
			this._text.style.height = height + "px";
			this.height = height + "px";
		} else {
