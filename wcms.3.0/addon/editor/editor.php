...
<link rel="stylesheet" href="<?php echo($cfg['droot']);?>addon/editor/css/editor.css" type="text/css" charset="<?php echo($cfg['charset']);?>" />
<!-- 에디터 컨테이너 시작 -->
<div id="tx_trex_container" class="tx-editor-container">
	...
	<div id="tx_sidebar" class="tx-sidebar">
			<div class="tx-sidebar-boundary">
					...
					<ul class="tx-bar tx-bar-right tx-group-undo" style="margin:4px 4px 0 0">
						<li class="tx-list">
							...
							<div id="tx_richtextbox_menu" class="tx-richtextbox-menu tx-menu">
									...
									<div class="tx-menu-footer">
											<img class="tx-menu-confirm"
													 src="/addon/editor/images/icon/editor/btn_confirm.gif?rv=1.0.1" alt=""/>
											<img class="tx-menu-cancel" hspace="3"
													 src="/addon/editor/images/icon/editor/btn_cancel.gif?rv=1.0.1" alt=""/>
									</div>
							</div>
					</li>
					<li class="tx-list">
...
<div id="tx_loading" class="tx-loading"><div><img src="<?php echo($cfg['droot']);?>addon/editor/images/icon/editor/loading2.png" width="113" height="21" align="absmiddle"/></div></div>
<div id="tx_canvas_wysiwyg_holder" class="tx-holder" style="display:block;">
	<iframe id="tx_canvas_wysiwyg" name="tx_canvas_wysiwyg" allowtransparency="true" frameborder="0"></iframe>
</div>
<div class="tx-source-deco">
	<div id="tx_canvas_source_holder" class="tx-holder">
			<textarea id="tx_canvas_source" rows="30" cols="30"><?php echo(stripslashes($Rows['content']));?></textarea>
	</div>
</div>
...
<img id="tx_resize_holder" src="<?php echo($cfg['droot']);?>addon/editor/images/icon/editor/skin/01/btn_drag01.gif" width="58" height="12" unselectable="on" alt="" />
</div>
			<!-- 편집영역 끝 -->
	<!-- 첨부박스 시작 -->
			<!-- 파일첨부박스 Start -->
...
<div id="tx_attach_box" class="tx-attach-box">
	<div class="tx-attach-box-inner">
			<div id="tx_attach_preview" class="tx-attach-preview"><p></p><img src="<?php echo($cfg['droot']);?>addon/editor/images/icon/editor/pn_preview.gif" width="147" height="108" unselectable="on"/></div>
...
var config = {
	...
	txPath: '<?php echo($cfg['droot']);?>addon/editor/', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) /xxx/xxx/ */
	txVersion: '5.4.0', /* 수정필요없음. */
	txService: 'builder', /* 수정필요없음. */
	txProject: 'builder', /* 수정필요없음. 프로젝트가 여러개일 경우만 수정한다. */
	initializedId: "", /* 대부분의 경우에 빈문자열 */
	wrapper: "tx_trex_container"+"", /* 에디터를 둘러싸고 있는 레이어 이름(에디터 컨테이너) */
	form: 'bbsform'+"", /* 등록하기 위한 Form 이름 */
	txIconPath: "<?php echo($cfg['droot']);?>addon/editor/images/icon/editor/", /*에디터에 사용되는 이미지 디렉터리, 필요에 따라 수정한다. */
	txDecoPath: "<?php echo($cfg['droot']);?>addon/editor/images/deco/contents/", /*본문에 사용되는 이미지 디렉터리, 서비스에서 사용할 때는 완성된 컨텐츠로 배포되기 위해 절대경로로 수정한다. */
	canvas: {
		initHeight: <?php echo(($editorHeight)?$editorHeight:'200'); ?>,
		styles: {
			color: "#333", /* 기본 글자색 */
...
EditorJSLoader.ready(function (Editor) { new Editor(config).focusOnForm("subject"); });
//에디터 이미지 팝업에 변수 전달하기
Editor.getConfig().sidebar.attacher.image.popPageUrl = '/addon/editor/image.html?skin=<?php echo($cfg['skin']);?>';
//에디터로 포커스되는 것을 방지하고
//아래 observe로 툴바 클릭시 IE에러를 방지한다.
Editor.getToolbar().observeJob(Trex.Ev.__TOOL_CLICK, function (type) { Editor.focus(); });
/*-------- 에디터 로드 끝 ----------*/
...
function submitForm(frm)
{
	...
	<?php
	if(!$_SESSION['ulevel'] || $_SESSION['ulevel'] > '2') {
	?>
