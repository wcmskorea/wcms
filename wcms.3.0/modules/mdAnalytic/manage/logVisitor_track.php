...
<ul>
	<li>검색 날짜의 한달 이전부터 누적 방문횟수가 "5회" 이하의 방문자 중 "페이지 이동 1회 이상자" 목록입니다.</li>
...
<dl>
	...
	<dd style="float:right; padding:1px;"><span class="btnPack green medium strong"><button type="submit" onclick="$.insert('#tabBody01', '../modules/mdAnalytic/manage/logVisitor_track.php', '&amp;year=' + $('#year').val() + '&amp;month=' + $('#month').val() + '&amp;day=' + $('#day').val(), 300)">검색</button></span></dd>
...
<table class="table_list" summary="유입 IP별 방문자 현황" style="width:100%;">
	...
	$query = " SELECT * FROM `mdAnalytic__track` WHERE DATE_FORMAT(date,'%Y-%m-%d') = '".$year."-".$month."-".$day."' AND counting > '0'  GROUP BY ip ORDER BY LENGTH(info) DESC ";
	$db->query($query);
	if($db->getNumRows() < 1)
	{
	...
	else
	{
		while($Rows = $db->fetch())
		{
			...
			//}
			if(intval(count($tracking)-1) < 3) { $black .= $Rows['ip']."\n"; }
			unset($recent, $per1);
		}
	}
	?>
...
<!--<div><textarea name="blackList"><?php echo($black); ?></textarea></div>-->