 ...
 * 리스트 본문 : Star
 */
echo('<div class="boardThumb"><div style="width:'.$sizeAll.'px; margin:auto;">');
$i = 0;
$n = 0;
...
while($Rows = $db->fetch()) {
	...
	$dir = $cfg['droot']."skin/".$cfg['site']['skin']."/data/".date("Y",$Rows['date'])."/".date("m",$Rows['date'])."/".$Rows['filename'];
	//리스트 출력
	...
	<div class="center">
		...
		<div style="width:'.$Rows['width'].'px;">'.$url.$Rows['subject'].'</a></div>
			<div class="nameAndDate">
				<div><span>'.$Rows['url'].'</span></div>
				<div><span>'.$Rows['name'].'</span></div>
			</div>
		</div>
	</div>');
	$n++;
