...
while($Rows	= @mysql_fetch_array($rst))
{
	...
	$width = ($Rows['width']) ? $Rows['width'].'px' : '100%';
	$height = ($Rows['height']) ? $Rows['height'].'px' : '100%';
	$Rows['url'] = ($Rows['url']) ? $Rows['url'] : "javascript:;";

	if($Rows['url'])
	{
		$this->result .= '<li class="bannerUnit"><a href="'.$Rows['url'].'"'.($Rows['target'] == 'none' || $Rows['target'] == '' ? '':' target="'.$Rows['target'].'"').'><img src="'.$dir.'" class="thumbNail" style="width:'.$width.'; height:'.$Rows['height'].'px;" onerror="this.src=\''.$this->cfg['droot'].'common/image/noimg_m.gif\'" onmouseover="overClass(this);" onmouseout="overClass(this);" onfocus="overClass(this);" onblur="overClass(this);" alt="'.$Rows['subject'].'" title="'.$Rows['subject'].'" /></a></li>'.PHP_EOL;
	} else {
		$this->result .= '<li class="bannerUnit"><img src="'.$dir.'" class="thumbNail" style="width:'.$width.'; height:'.$Rows['height'].'px;" onerror="this.src=\''.$this->cfg['droot'].'common/image/noimg_m.gif\'" onmouseover="overClass(this);" onmouseout="overClass(this);" onfocus="overClass(this);" onblur="overClass(this);" alt="'.$Rows['subject'].'" title="'.$Rows['subject'].'" /></li>'.PHP_EOL;
	}
	$n++;
}

$this->result .= '</ul><div class="clear"></div></div>'.PHP_EOL;
$this->result .= '<script type="text/javascript">$(document).ready(function(){$("#banner'.$this->prefix.'").bxSlider({mode:"fade",easing:"'.$this->config['docEasing'].'",controls:'.$this->config['docBtn'].',pager:'.$this->config['docPager'].',controls:'.$this->config['docBtn'].',auto:'.$this->config['docStart'].',speed:'.$this->config['docSpeed'].',pause:'.$this->config['docDelay'].',ticker:'.$this->config['docType'].',tickerSpeed:'.$this->config['docDelay'].',displaySlideQty:'.$this->config['docCount'].',captions:'.$this->config['docCaption'].'});});</script>'.PHP_EOL;
