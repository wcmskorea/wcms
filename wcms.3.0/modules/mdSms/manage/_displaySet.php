...
//		$func->validCheck(체크할 값, 항목제목, 항목명, 체크타입)
	...
	if($db->sqlInsert("display__".$skin, "REPLACE", 0) > 0)
	{
		...
		{
			$func->errCfm("Display(SMS모듈) 위젯 정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#skinSelector', '".$cfg['droot']."_Admin/modules/displayList.php?type=displayList&mode=design&skin=".$_POST['skin']."&position=".$_POST['position']."');");
		}
		else
		{
			$func->err("Display(SMS모듈) 위젯 정보가 정상적으로 적용되었습니다.", "parent.$.dialogRemove();".PHP_EOL."parent.$.insert('#tabBody".$_POST['position']."', './modules/displayList.php?type=displayList&skin=".$_POST['skin']."&position=".$position."',null,300);");
		}
	}
}
//디스플레이 수정
...
<table class="table_list" summary="Display(게시판관리 모듈) 유닛설정">
	...
	$form->add('select', array('Simple'=>'심플형 (이름,연락처)'), $Rows['listing'], 'width:170px;');//'Phone'=>'휴대폰형 (이름,연락처)'
	$form->addEnd(0);
