...
if($_GET['division'] != '') $Rows['division'] = $_GET['division']; // 구분값 전달시 해당 구분값이 선택되어지도록 처리(2013-07-22)
...
} else if($_SESSION['uid'] && $_SESSION['ulevel'] > $cfg['operator'])
{
	$Rows = $db->queryFetch(" SELECT * FROM `mdMember__account` AS A,`mdMember__info` AS B WHERE A.id=B.id AND B.id='".$_SESSION['uid']."' ");
}

//상담내역 자동 삽입
$content = ($Rows['content']) ? $Rows['content'] : $_GET['content'];
?>
<div class="boardInput">

<form id="bbsform" name="bbsform" method="post" action="<?php echo($_SERVER['PHP_SELF']);?>?mode=<?php echo($_GET['mode']);?>" enctype="multipart/form-data">
...
<input type="hidden" name="fileCount" value="<?php echo($Rows['file']);?>" />
    ...
    /*
	if($cfg['module']['opt_idcode'] != 'N') {
		$form->addStart('주민번호', 'idcode', 1, 0, $cfg['module']['opt_idcode']);
		$form->add('input', 'idcode', $Rows['idcode'], 'width:120px;', 'idcode="true"');
		$form->addHtml('<li class="opt"><span class="small_gray"> (예: 19880101-1234567)</span></li>');
		$form->addEnd(1);
	}
    */
	...
	if($cfg['module']['opt_address'] && $cfg['module']['opt_address'] != 'N') {
		...
		$form->add('input', 'zipcode', $Rows['zipcode'], 'width:120px;', 'maxlength="7" zipno="true"');
	...
	/*for($i=1; $i<=20; $i++)
	{
		if($cfg['module']['opt_add'.$i] != 'N')
		{
			...
			//$contentAdd['addContent'.$i] = ($contentAdd['addContent'.$i]) ? $contentAdd['addContent'.$i] : $_GET['addContent'.$i];
	...
	if($cfg['module']['opt_content'] != 'N') {
		...
		$form->add('textarea', 'content', stripslashes(nl2br($content)), 'width:400px; height:100px;');
		$form->addEnd(1);
	}
	?>
	</table>
...
if(!$Rows || $Rows['state'] < 1)
{
	...
	} else
	{
		...
		echo('<div class="pd10 center"><span class="btnPack black medium strong"><button type="submit" '.$uploadSubmit.'>위 정보로 신청합니다</button></span>&nbsp;&nbsp;<a href="#none" onclick="$.dialog(\''.$cfg['droot'].'index.php\', \'&amp;'.__PARM__.'&amp;type=search&amp;mode=dialog&amp;moded='.$_GET['mode'].'\',400,165)" class="btnPack green medium strong"><span>신청내역 열람 및 변경신청</span></a></div>');
