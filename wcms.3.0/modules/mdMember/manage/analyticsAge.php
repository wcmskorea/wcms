...
<div class="table"><div class="line">
	<fieldset id="help">
	<legend>→ 연령별 통계 ←</legend>
	<ul>
		<li>사이트 Launching 이후 사이트 연령별 통계입니다.</li>
...
<dl>
	...
	<dd style="float:right; padding:1px;"><span class="btnPack green medium strong"><button type="submit" onclick="$.insert('#module', '../modules/mdMember/manage/_controll.php', '&amp;type=analyAge&amp;year=' + $('#year').val() + '&amp;month=' + $('#month').val(), 300)">검색</button></span></dd>
...
<table class="table_list" summary="연령별 사이트 가입자 현황" style="width:100%;">
	...
	$db->Query("SELECT SUBSTRING((age+10),1,1) AS ageGroup,COUNT(*) AS total FROM `mdMember__info` A, `mdMember__account` B  WHERE A.id = B.id AND ".$sq." GROUP BY ageGroup ORDER BY total DESC");
	if($db->getNumRows() < 1)
	{
	...
	} else
	{
		while($Row = $db->Fetch())
		{
			...
			$ageGroup = (intval($Row[ageGroup]-1)) ? intval($Row[ageGroup]-1).'0대' : "미입력";
			print('<tr>
        <th><p class="center">'.$ageGroup.'</p></th>
