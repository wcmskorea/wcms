...
<table class="table_list" summary="지역별 사이트 가입자 현황" style="width:100%;">
	...
	$db->Query("SELECT SUBSTRING(address01,1,6) AS regionGroup,COUNT(*) AS total FROM `mdMember__info` A, `mdMember__account` B  WHERE A.id = B.id AND ".$sq." GROUP BY regionGroup ORDER BY total DESC");
	if($db->getNumRows() < 1)
	{
