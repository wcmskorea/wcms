...
<tr>
  ...
  <td style="vertical-align:top;">
    ...
    $i = 0;
		$db->query(" SELECT substring(login,1,10) as login_date, ip, count(seq) as login_cnt  FROM `mdMember__log` WHERE id='".$_GET['user']."' GROUP BY login_date, ip ORDER BY login_date ASC LIMIT ".$rows );
    ...
    $rows	= 10;
    $i		= 0;
		$query = "SELECT * FROM `mdAnalytic__refer` WHERE ip='".$userip."' GROUP BY ip,referer ORDER BY date DESC";
		$db->query($query);
    while($sRows = $db->Fetch())
    {
