...
$config = $db->queryFetch(" SELECT cate,articled FROM `mdDocument__` WHERE boardType='HUGIBASIC' limit 1 ");

$sql  = 'productSeq="'.$_GET['num'].'" AND boardType="HUGIBASIC"';
...
echo('<div class="docBtn"><ul>');
	//if($_SESSION['uid'])
	//{
		echo('<li><span class="btnPack black medium strong"><a href="javascript:;" onclick="new_window(\''.$cfg['droot'].'?cate='.$config['cate'].'&amp;type='.$sess->encode("input").'&amp;mode=content&amp;productSeq='.$productSeq.'\',\'contentWcms\',800,600,\'no\',\'yes\');">이용후기 작성하기</a></span></li>');
	//}
...
echo('<th scope="col" class="first"><p class="center pd7">'.$lang['doc']['num'].'</p></th>
	<!--<th scope="col"><p class="center pd7">'.$lang['doc']['productinfo'].'</p></th>-->
	<th scope="col"><p class="center pd7">'.$lang['doc']['subject'].'</p></th>
...
while($Rows = $db->fetch())
{
	...
	$url 			= '<span><a href="javascript:;" onclick="f_ClickShow(\'divHugi_'.$articleNum.'\')">';

	if($Rows['useSecret'] == "Y")
	{
		$icon .= '<span><img src="'.$cfg['droot'].'user/default/image/icon/secret.gif" alt="비밀글" /></span>';
		/*if($_SESSION['ulevel'] < $cfg['operator'] || ($Rows['id'] == $_SESSION['uid'] && $_SESSION['uid'])) {
			$url   = '<span><a href="javascript:;" onclick="f_ClickShow(\'divHugi_'.$articleNum.'\')">';
		}else{
	...
	$hugiImg = '<img width="50" border="0" src="'.str_replace("/l-", "/s-", $imgDir[0]).'" align=absmiddle />';
	echo('<tr>
					<td scop="row"><p class="center">'.$articleNum.'</p></td>
					<!--<td><p>'.$hugiImg.'<span style=margin-left:10px;>'.$productInfo['productName'].'</span></td>-->
				...
				<tr id="divHugi_'.$articleNum.'" style="display:none">
					<td scop="row" colspan="4" bgcolor="f5f5f5">
					<div class="contentBody textContent">'.$func->matchImage($content).'</div>');
					//본인글만 수정하기 버튼 출력
					if($Rows['id'] == $_SESSION['uid'])
					{
						echo('<div style="clear:both; float:right"><span class="btnPack gray medium"><a href="'.$_SERVER['PHP_SELF'].'?cate='.$config['cate'].'&amp;type='.$sess->encode('articleModify').'&amp;num='.$Rows['seq'].'">수정하기</a></span></div>');
					}
...
if($n < 1)
{
	echo('<tr><td colspan="4" class="blank">등록된 이용후기가 없습니다.</td></tr>');
}
