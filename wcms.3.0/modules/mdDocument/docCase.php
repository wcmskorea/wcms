...
echo('<div class="clear"></div><div class="docInfo">');
	...
	echo('<div class="docBtn"><ul>');
		...
		{
			echo('<li><span class="btnPack small strong"><a href="'.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type='.$sess->encode("input").'">'.$lang['doc']['write'].'</a></span></li>');
		}
		if($member->checkPerm('0') && $type != 'view' && $cfg['module']['listUnion'] != 'Y')
		{
			echo('<li><span class="btnPack gray small"><button type="button" onclick="$.dialog(\''.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type=move&amp;mode=dialog\',null,300,140)">'.$lang['doc']['move'].'</button></span></li>');
			echo('<li><span class="btnPack gray small"><button type="button" onclick="$.dialog(\''.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type=clear&amp;mode=dialog\',null,300,140)">'.$lang['doc']['clear'].'</button></span></li>');
		}
		if($member->checkPerm('s') && $type != 'view')
		{
			echo('<li><span class="btnPack black small"><a href="'.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type='.$sess->encode('articleOptimize').'#procForm" class="button small bblack">'.$lang['doc']['optimize'].'</a></span></li>');
			echo('<li><span class="btnPack black small"><a href="'.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type='.$sess->encode('articleReset').'#procForm" class="button small bblack">'.$lang['doc']['reset'].'</a></span></li>');
		}
...
//	데이터 정리
	...
	$url 			= '<span><a href="'.$_SERVER['PHP_SELF'].'?'.__PARM__.'&amp;type=view&amp;num='.$Rows['seq'].'#module">';
	if($Rows['useSecret'] == "Y")
	{
	...
	//하위자식 최신글 조회하여 자식글중 이미지로 목록 이미지 대체
	$subRows = $db->queryFetch("SELECT * FROM `".$cfg['cate']['mode']."__content".$prefix."` WHERE cate='".$cfg['module']['cate']."' AND idxTrash='0' AND productSeq='".$Rows['seq']."' AND fileCount>0 ORDER BY idx DESC Limit 1", 3);
	
	if($subRows['seq']) {
		$icon 			.= $func->iconNew($subRows['regDate'], (86400*3), '<span><img src="'.$cfg['droot'].'user/default/image/icon/new.gif" alt="최근 게시물" /></span>');

		$subThumb = $db->queryFetch("SELECT * FROM `".$cfg['cate']['mode']."__file".$prefix."` WHERE parent='".$subRows['seq']."' AND LOWER(extName) IN ('jpg','gif','png','bmp') ORDER BY seq ASC Limit 1", 4);
		if($subThumb['fileName'])
			$dir = str_replace($cfg['site']['lang']."/",null,__SKIN__)."data/".date("Y",$subThumb['regDate'])."/".date("m",$subThumb['regDate'])."/m-".$subThumb['fileName'];
	}
	...
	<div class="center" style="width:'.$cfg['module']['thumbMsize'].'px;">
		...
		<div class="nameAndDate" style="margin-bottom:10px;">
			<div class="date"><span>'.$date.'</span>'.($cfg['module']['readCount'] != 'N' ? ' / hit: <span class="num">'.$Rows['readCount'].'</span>':'').'</div>
