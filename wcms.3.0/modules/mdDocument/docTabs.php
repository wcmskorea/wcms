...
echo('</div><div class="clear"></div>').PHP_EOL;
?>
...
while($Rows = $db->fetch())
{
	...
	$commentCount		= ($Rows['commentCount']) ? '&nbsp;<span class="small_red strong">('.$Rows['commaentCount'].')</span>' : null;
...
while($Rows = $db->fetch())
{
	...
	if($cfg['module']['opt_division'] != 'N')
	{
		$Rows['subject'] = preg_replace("#\[(.*?)\]#si", '<span class="colorActive">\\0</span>', $Rows['subject']);
	}
	if($Rows['idxDepth'])
	{
	...
	echo('<tr>
		...
		<td scop="row"><dl><dt>'.$check.$url.'<strong>'.$Rows['subject'].'</strong>'.$commentCount.'</a></span></dt>
