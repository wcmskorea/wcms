...
while($Rows = $db->fetch())
{
	...
	echo('<tr class="bg3">
		...
		<td><p class="wrap80"><strong class="colorBlack">'.$Rows['writer'].'<strong></p></td>');
 ...
 */
$n = 0;
echo("<!-- SELECT * FROM `".$cfg['cate']['mode']."__content".$prefix."` WHERE ".$sql." ".$pagingResult['LimitQuery'] ."-->");
...
while($n < $row)
{
	echo('<tr class="bg1">
	    ...
	    <td><p class="center pd4 silver">-</p></td>
	</tr>');
	$n++;
}
...
echo('<div class="docBottom">
	...
	<div class="searchBox">
	    ...
	    <div class="selectBox" style="width:90px;">
			<span class="selectCtrl"><span class="selectArrow"></span></span>
			<div class="selectValue">'.$lang['search']['subject'].'</div>
			<ul class="selectList2">
			<li><input name="sh" id="sh0" type="radio" value="subject" checked="checked" class="option"><label for="sh0">'.$lang['search']['subject'].'</label></li>
			<li><input name="sh" id="sh1" type="radio" value="all" class="option"><label for="sh1">'.$lang['search']['content'].'</label></li>
			<li><input name="sh" id="sh2" type="radio" value="writer" class="option"><label for="sh2">'.$lang['search']['writer'].'</label></li>
			</ul>
		</div>
