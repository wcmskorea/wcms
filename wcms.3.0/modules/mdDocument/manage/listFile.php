...
$articleCount	= $db->queryFetchOne(" SELECT COUNT(*) FROM `mdDocument__file` AS A INNER JOIN `mdDocument__content` AS B ON A.parent = B.seq AND 1".$sq." ");
...
?>
<h2><span class="arrow">▶</span>문서·게시물 관리&nbsp;&nbsp;<span class="normal">>&nbsp;등록된 파일</span></h2>
<div class="table">
...
<div class="selectBox" style="width:100px;">
	...
	<ul class="selectList1">
		<li><a href="javascript:;" onclick="$.insert('#module', '../modules/mdDocument/manage/_controll.php?type=file&amp;cated=<?php echo($cated)?>&amp;row=15',null,300);">15건씩 보기</a></li>
		<li><a href="javascript:;" onclick="$.insert('#module', '../modules/mdDocument/manage/_controll.php?type=file&amp;cated=<?php echo($cated)?>&amp;row=30',null,300);">30건씩 보기</a></li>
		<li><a href="javascript:;" onclick="$.insert('#module', '../modules/mdDocument/manage/_controll.php?type=file&amp;cated=<?php echo($cated)?>&amp;row=50',null,300);">50건씩 보기</a></li>
		<li><a href="javascript:;" onclick="$.insert('#module', '../modules/mdDocument/manage/_controll.php?type=file&amp;cated=<?php echo($cated)?>&amp;row=100',null,300);">100건씩 보기</a></li>
	</ul>
</div>
<div class="selectBox" style="width:300px;">
	...
	<ul class="selectList1">
		...
		while($sRows = $db->fetch()) 
		{
			...
			echo('<li><a href="javascript:;" onclick="$.insert(\'#module\', \'../modules/mdDocument/manage/_controll.php?type=file&amp;row='.$row.'&amp;cated='.$sRows['cate'].'\',null,300);">'.$blank.' ('.substr($sRows['cate'],-3).')'.$sRows['name'].'</a></li>');
			if($sRows['cate'] == $cated)
			{
...
<table class="table_basic" style="width:100%; margin-top:5px;">
	...
	<thead>
		<tr>
			...
			<th><p class="center">종류</p></th>
			<th><p class="center">원본 파일명</p></th>
			<th><p class="center">저장 파일명</p></th>
			<th><p class="center">등록일</p></th>
		</tr>
	</thead>
	<tbody>
	<?php
	$db->query(" SELECT *,A.regDate AS regDate FROM `mdDocument__file` AS A INNER JOIN `mdDocument__content` AS B ON A.parent = B.seq AND 1".$sq." ORDER BY B.seq DESC ".$pagingResult['LimitQuery'] );

	if($db->getNumRows() < 1) 
	{
		echo('<tr><td class="blank" colspan="7">등록된 문서의 첨부파일이 존재하지 않습니다.</td></tr>');
	} 
	else 
	{
		while($Rows = $db->fetch()) 
		{
			...
			<td class="bg_gray"><p class="center"><a href="'.$cfg['droot'].'addon/system/download.php?'.__PARM__.'&amp;file='.$sess->encode($dir).'&name='.$Rows['realName'].'"><span><img src="'.$cfg['droot'].'common/image/files/'.strtolower($Rows['extName']).'.gif" align="absmiddle" onError="this.src=\''.$cfg['droot'].'common/image/files/unKonwn.gif\'" width="16" height="16" /></span></a></p></td>
			<td><p>'.$Rows['realName'].'</a></p></td>
			<td><p>'.$Rows['fileName'].'</p></td>
