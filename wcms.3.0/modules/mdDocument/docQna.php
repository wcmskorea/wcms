...
while($Rows = $db->fetch())
{
	...
	echo('<tr>
				...
				<tr id="divContent_'.$articleNum.'" style="display:none">
					...
					{
						echo('<div style="float:right"><span class="btnPack gray medium"><a href="'.$_SERVER['PHP_SELF'].'?cate='.$config['cate'].'&amp;type='.$sess->encode('articleModify').'&amp;num='.$Rows['seq'].'">수정하기</a></span></div>');
					}
	echo('</td></tr>');
