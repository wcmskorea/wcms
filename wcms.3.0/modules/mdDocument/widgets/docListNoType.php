...
while($Rows = @mysql_fetch_assoc($rst))
{
	...
	$subject = $Rows['subject']; //str_replace($out[1][0], '<span style="color:'.$GLOBALS['cfg']['docHeadColor'][$out[1][0]].'">'.$out[1][0].'</span>', $Rows['subject']);
	...
	$this->result .= '<li class="docTitle" style="width:'.$subjectWidth.'px; margin:0 '.$this->config['docPad'].'; '.($this->config['docPadT'] ? 'padding-top:'.$this->config['docPadT'] : '').';"><span class="no">'.($n+1).'</span>'.$url.$subject.'</a><span class="date" style="right:'.$this->config['docPad'].';">'.$date.'</span></li>'.PHP_EOL;
	$n++;
}

/*while($n < $this->config['docCount'])
{
	$this->result .= '<li class="docTitle" style="width:'.$subjectWidth.'px; margin:0 '.$this->config['docPad'].';"><span class="no">'.($n+1).'</span><span class="title" style="color:#999;">...</span><span class="date" style="right:'.$this->config['docPad'].';"></span></li>'.PHP_EOL;
	$n++;
}*/
