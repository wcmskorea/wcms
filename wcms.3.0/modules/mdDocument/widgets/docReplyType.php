...
while($Rows = @mysql_fetch_assoc($rst))
{
	...
	//답변수
	$replyCnt = @mysql_fetch_array(@mysql_query(" SELECT COUNT(*) FROM `".$this->config['module']."__content` WHERE productSeq ='".$Rows['seq']."'"));
	//답변여부
	if($replyCnt[0] > 0 || $Rows['commentCount'] > 0)
	...
	if($this->config['docLink'] == 'L')  //컨텐츠 클릭시 목록으로 이동 추가(20120522)
		$url 			= '<a href="./?cate='.$Rows['cate'].'" title="'.$Rows['subject'].'" target="'.$this->target.'">';
	else {
		if($Rows['useSecret'] == "Y"){ $typeString = 'type='.$GLOBALS['sess']->encode('secret');}
		else{ $typeString = "type=view";}
		$url = '<a
	href="./?cate='.$Rows['cate'].'&amp;'.$typeString.'&amp;num='.$Rows['seq'].'&amp;productNum='.$Rows['productSeq'].'" title="'.$Rows['subject'].'" target="'.$this->target.'">';
	}
	$this->result .= '<li class="docTitle" style="width:'.$subjectWidth.'px; margin:0 '.$this->config['docPad'].'; '.($this->config['docPadT'] ? 'padding-top:'.$this->config['docPadT'] : '').';"><span class="title">'.$url.$subject.'</a></span>'.$icon;
	$this->result .= '<span class="date" style="right:'.$this->config['docPad'].';">'.$status.'</span>';
	$this->result .= ($this->config['useDate'] == 'Y') ? '<span class="date" style="right:'.$this->config['docPad'].';">'.$date.'</span>':'';
...
{
	$this->result .= '<li class="docTitle" style="width:'.$subjectWidth.'px; margin:0 '.$this->config['docPad'].';"><span class="title" style="color:#999;">...</span>'.$icon.'<span class="date" style="right:'.$this->config['docPad'].';"></span></li>'.PHP_EOL;
	$n++;
}
