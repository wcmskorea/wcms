...
while($Rows	= @mysql_fetch_assoc($rst))
{
	...
	if($contentAdd[0] != '') {$this->result .= '<div class="nameAndDate"><p class="date" style="width:'.$this->config['imgWidth'].';">'.$contentAdd[0].'</p></div>'.PHP_EOL; }
	$this->result .= '</li>'.PHP_EOL;
	$n++;
}
while($n < $this->config['docCount'])
{
	$this->result .= '<li class="cell" style="padding-top:'.$this->config['docPadT'].'; padding-left:'.$this->config['docPad'].';"><div style="position:relative;"><a href="#none"><img src="'.$GLOBALS['cfg']['droot'].'common/image/noimg_s.gif" alt="준비중" class="thumbNail" style="width:'.$this->config['imgWidth'].'; height:'.$this->config['imgHeight'].';" /></a></div>';
