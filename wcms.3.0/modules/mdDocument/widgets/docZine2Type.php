...
$this->result .= '<div class="recentZine" style="width:'.$this->config['width'].'; height:'.$this->config['height'].'; background:url('.__SKIN__.'image/background/bg_recent_'.strtolower($this->prefix).'.gif) no-repeat left top;">'.PHP_EOL;

$n = 0;
$rst = @mysql_query(" SELECT * FROM `".$this->config['module']."__content` WHERE ".$squery);
//$this->result .= '<!-- mysql_query'." SELECT * FROM `".$this->config['module']."__content` WHERE ".$squery.'-->';
while($Rows	= @mysql_fetch_assoc($rst))
{
	...
	$this->result .= '<div class="box" style="padding:'.$this->config['docPad'].';"><a href="'.$url.'" title="'.$Rows['subject'].'" target="'.$this->target.'"><img src="'.$dir.'" alt="'.$thumb['fileName'].'" class="thumbNail" style="width:'.$this->config['imgWidth'].'; height:'.$this->config['imgHeight'].';" onerror="this.src=\''.$GLOBALS['cfg']['droot'].'common/image/noimg_m.gif\'" onmouseover="overClass(this);" onmouseout="overClass(this);" /></a><a href="'.$url.'" title="'.$Rows['subject'].'" target="'.$this->target.'" class="link"><h6>'.$subject.'</h6><p style="width:'.$contentWidth.'px;">'.$Rows['content'].'</p></a></div>'.PHP_EOL;
	$n++;
}
/*
