...
while($Rows	= @mysql_fetch_assoc($rst))
{
	...
	$date = date("Y.m.d", $Rows['regDate'])." ~ ".date("Y.m.d", ($Rows['endDate']-60));
	//링크설정
	...
	//내용출력
	$this->result .= '<li style="padding:'.$this->config['docPad'].' '.$this->config['docPad'].' 0 '.$this->config['docPad'].';"><p class="headLine">'.$url;
	if($thumb) 
	{
		$this->result .= '<span class="imgList"><img src="'.$dir.'" alt="'.$thumb['fileName'].'" class="thumbNail" style="width:'.$this->config['imgWidth'].'; height:'.$this->config['imgHeight'].';" onerror="this.src=\''.$GLOBALS['cfg']['droot'].'common/image/noimg_m.gif\'" onmouseover="overClass(this);" onmouseout="overClass(this);" /></span>';
		$this->result .= '<strong>'.$Rows['subject'].'</strong></a></p><p class="headLine">참여기간: <strong>'.$date.'</strong></a></p><p class="content" style="width:'.$contentWidth.'px;">'.$Rows['content'].'</p>';
	} else {
		$this->result .= '<strong>'.$Rows['subject'].'</strong></a></p><p class="headLine">참여기간: <strong>'.$date.'</strong></a></p><p class="content">'.$Rows['content'].'</p>';
	}
	//참여버튼 출력
	if($Rows['endDate'] > time())
	{
		$this->result .= '<p class="right"><span class="btnPack small green">'.$url.'참여하기</a></span></p></li>'.PHP_EOL;
	} else {
		$this->result .= '<p class="right"><span class="btnPack small gray"><a>종료</a></span></p></li>'.PHP_EOL;
	}
	$n++;
}
