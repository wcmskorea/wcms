...
if($cfg['module']['list'] != 'Page')
{
	...
	if($cfg['module']['listUnion'] == 'Y')
	{
		echo('<tr>
			...
			$db->query(" SELECT A.cate AS cate,B.name AS name FROM `".$cfg['cate']['mode']."__` AS A INNER JOIN `site__` AS B ON A.cate=B.cate AND UBSTRING(A.cate,1,".strlen($cfg['module']['cate']).")='".$cfg['module']['cate']."' AND A.listUnion<>'Y' ORDER BY A.cate ASC ");
	...
	if($cfg['module']['addContent'])
	{
		...
		foreach($addOpt AS $key=>$val)
		{
			...
			$form->add('textarea', 'addopt['.$key.']', $contentAdd[$key], 'width:390px; height:80px;');
			$form->addEnd(1);
		}
	}
...
jQuery(function($)
{
	$("#redate").click(function()
	{
		if($("#redate:checked").length > 0)
		{
			...
			$("#reday").val(thisDate.getDay()).css('color','red');
