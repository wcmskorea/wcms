...
if($db->sqlInsert("display__".$skin, "REPLACE", 0) > 0)
{
	...
	{
		$func->ajaxMsg("Display(Clear) 위젯 정보가 정상적으로 적용되었습니다.", "$.insert('#skinSelector', '".$cfg['droot']."_Admin/modules/displayList.php?type=displayList&mode=design&skin=".$_GET['skin']."&position=".$_GET['position']."',null,300)", 20);
	}
	else
	{
		$func->ajaxMsg("Display(Clear) 위젯 정보가 정상적으로 적용되었습니다.", "$.insert('#tabBody".$_GET['position']."', './modules/displayList.php?type=displayList&skin=".$_GET['skin']."&position=".$_GET['position']."',null,300)", 20);
	}
}
?>
